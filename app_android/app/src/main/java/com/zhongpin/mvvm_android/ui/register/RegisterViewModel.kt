...
class RegisterViewModel  : BaseViewModel<RegisterRepository>() {
    ...
    fun register(mobile:String, password:String, code:String, nick:String) : MutableLiveData<BaseResponse<Boolean>> {
        val mLoginData: MutableLiveData<BaseResponse<Boolean>> = MutableLiveData()
        initiateRequest({
            mLoginData.value = mRepository.register(mobile, password, code, nick);
        }, loadState)
        return mLoginData
    }
